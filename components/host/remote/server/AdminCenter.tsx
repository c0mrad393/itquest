 ...
 * server has no directory, so it has no Active Directory tab — nothing is
...
type SectionId = "overview" | "directory" | "shares" | "services" | "events";
...
const SECTIONS: Section[] = [
  ...
  { id: "directory", label: "Active Directory", iconId: "users", when: (n) => !!n.activeDirectory },
  { id: "shares", label: "File Shares", iconId: "folder", when: (n) => !!n.shares?.length },
...
export default function AdminCenter({ nodeId }: { nodeId: string }) {
  ...
  return (
    <div className="flex h-full bg-panel text-gray-200">
      {/* ── Sidebar ──────────────────────────────────────────────────────── */}
      <nav className="flex w-52 shrink-0 flex-col border-r border-edge bg-panelalt/40">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-1.5">
          {available.map((s) => (
            <button
              ...
              className={`mb-0.5 flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[11px] transition ${
                active === s.id
                  ? "bg-info/15 text-gray-100"
        ...
        <div className="border-t border-edge px-3 py-2 text-[9px] leading-relaxed text-gray-600">
          Windows Admin Center · this session is bound to {node.hostname}
        </div>
      </nav>
